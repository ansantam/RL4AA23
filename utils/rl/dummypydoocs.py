...
class DummyMachine:
    ...
    def read(self, channel):
        ...
        # 4 correcotrs in the li
            ...
            channel == "SINBAD.DIAG/CAMERA/AR.EA.BSC.R.1/BINNINGHORIZONTAL"
        ):  # Screen horizontal binning
            data = self.segment.AREABSCR1.binning
        elif (
            channel == "SINBAD.DIAG/CAMERA/AR.EA.BSC.R.1/BINNINGVERTICAL"
        ):  # Screen vertical binning
            data = self.segment.AREABSCR1.binning
        elif (
            channel == "SINBAD.DIAG/CAMERA/AR.EA.BSC.R.1/WIDTH"
        ):  # Screen image width (pixel)
            data = int(
                self.segment.AREABSCR1.resolution[0] / self.segment.AREABSCR1.binning
            )
        elif (
            channel == "SINBAD.DIAG/CAMERA/AR.EA.BSC.R.1/HEIGHT"
        ):  # Screen image height (pixel)
            data = int(
                self.segment.AREABSCR1.resolution[1] / self.segment.AREABSCR1.binning
            )
        elif channel == "SINBAD.DIAG/CAMERA/AR.EA.BSC.R.1/X.POLY_SCALE":
            data = (
                np.array([0.0, 0.0, -self.segment.AREABSCR1.pixel_size[0], 0.0]) * 1000
            )
        elif channel == "SINBAD.DIAG/CAMERA/AR.EA.BSC.R.1/Y.POLY_SCALE":
            data = (
                np.array([0.0, 0.0, -self.segment.AREABSCR1.pixel_size[1], 0.0]) * 1000
            )
